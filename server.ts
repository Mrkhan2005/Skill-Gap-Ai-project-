...
import { users, resumeAnalyses, chatMessages } from './src/db/schema.ts';
...
// -------------------------------------------------------------
// Database Endpoints: User Management and Data Persistence
// -------------------------------------------------------------

// Sync user session to SQL database upon successful Google Login
app.post('/api/sync-user', requireAuth, async (req: AuthRequest, res) => {
  try {
    const { uid, email, name } = req.user!;
    const dbUser = await getOrCreateUser(uid, email, name || req.body.name);
    res.json({ status: 'success', user: dbUser });
  } catch (err: any) {
    console.error('Error in /api/sync-user:', err);
    res.status(500).json({ error: 'Database synchronization failed', message: err.message });
  }
});

// Retrieve saved resume/career analysis reports
app.get('/api/resume-analyses', requireAuth, async (req: AuthRequest, res) => {
  try {
    const { uid } = req.user!;
    const userRecords = await db.select().from(users).where(eq(users.uid, uid)).limit(1);
    if (!userRecords || userRecords.length === 0) {
      return res.status(404).json({ error: 'User profile not synchronized in target Cloud SQL database.' });
    }
    
    const reports = await db.select().from(resumeAnalyses)
      .where(eq(resumeAnalyses.userId, userRecords[0].id))
      .orderBy(desc(resumeAnalyses.createdAt));
      
    res.json(reports);
  } catch (err: any) {
    console.error('Error fetching resume analyses:', err);
    res.status(500).json({ error: 'Failed to retrieve saved analyses from Cloud SQL' });
  }
});

// Save a newly analyzed career intelligence profile
app.post('/api/save-analysis', requireAuth, async (req: AuthRequest, res) => {
  try {
    const { uid } = req.user!;
    const { fileName, analysisResult } = req.body;
    
    if (!fileName || !analysisResult) {
      return res.status(400).json({ error: 'Missing fileName or analysisResult in request body' });
    }
    ...
    const resultStr = typeof analysisResult === 'string' ? analysisResult : JSON.stringify(analysisResult);

    const inserted = await db.insert(resumeAnalyses)
      .values({
        userId: userRecords[0].id,
        fileName,
        analysisResult: resultStr
      })
      .returning();

    res.json({ status: 'success', saved: inserted[0] });
  } catch (err: any) {
    console.error('Error saving resume analysis:', err);
    res.status(500).json({ error: 'Failed to persist analysis to Cloud SQL database' });
  }
});

// Retrieve conversational coaching advice history
app.get('/api/chat-history', requireAuth, async (req: AuthRequest, res) => {
  try {
    const { uid } = req.user!;
    const userRecords = await db.select().from(users).where(eq(users.uid, uid)).limit(1);
    if (!userRecords || userRecords.length === 0) {
      return res.status(404).json({ error: 'User session not matched' });
    }

    const messages = await db.select().from(chatMessages)
      .where(eq(chatMessages.userId, userRecords[0].id))
      .orderBy(chatMessages.createdAt);

    res.json(messages);
  } catch (err: any) {
    console.error('Error fetching chat history:', err);
    res.status(500).json({ error: 'Failed to retrieve conversational history' });
  }
});

// Save a new chat advice item
app.post('/api/save-chat', requireAuth, async (req: AuthRequest, res) => {
  try {
    const { uid } = req.user!;
    const { role, content } = req.body;

    if (!role || !content) {
      return res.status(400).json({ error: 'Missing role or content parameters' });
    }
    ...
    console.error('Error saving coaching chat message:', err);
    res.status(500).json({ error: 'Failed to persist messaging records to Cloud SQL' });
  }
});
...
app.post('/api/analyze-resume', async (req, res) => {
  ...
  try {
    const prompt = `
      ...
      CRITICAL LATENCY & SPEED OPTIMIZATION:
      To ensure near-instant response times:
      - Be extremely concise in your JSON outputs.
      - Keep text fields, names, titles, descriptions, and suggestions short (under 10 words per field where appropriate).
      - Do not include conversational fluff.
      - Limit array lengths (such as 'skills', 'experience', 'projects', 'missingSkills', 'emergingSkills', 'decliningSkills', 'certifications', 'roadmap portfolioTasks', and 'jobMatches') to a high-quality top 3 items.
      - Use brief, impact-focused phrases instead of verbose paragraphs.
    `;
...
app.post('/api/chat-coach', async (req, res) => {
  ...
  try {
    ...
    const systemInstruction = `
      ...
      RESPONSE SPEED & CONCISENESS REQUIREMENT:
      - Keep answers highly focused and direct.
      - Avoid repeating previous details in long introductions.
      - Deliver the core advice in 2-3 short, clean structured paragraphs or direct bullet points.
      - Keep total word count under 200 words for near-instant, lightning-fast response speeds.
    `;
